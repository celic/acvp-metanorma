
[[app-reg-ex]]
== Example RSA Capabilities JSON Objects

This appendix contains example JSON object advertising support for the verious RSA modes:keyGen, sigGen, sigVer, legacySigVer, compRSASP1. Note that all binary HEX representations are in big-endian format.


[[app-ex-keyGenProvPrime]]
=== Example keyGen with Provable Primes and Provable Primes with Conditions Capabilities JSON Objects

The following is an example JSON object advertising support for RSA keyGen with provable primes according to
                    <<FIPS186-4>>, Appendix B.3.2, and provable primes with conditions according to
                    <<FIPS186-4>>, Appendix B.3.4. See also
                    <<mode_keyGenFullSet>>.


[align=left,alt=,type=]
....

   {
      "algorithm": "RSA",
      "mode": "keyGen",
      "revision": "1.0",
      "prereqVals": [{"algorithm": "DRBG", "valValue": "1234"}, {"algorithm": "SHA", "valValue": "5678"}],
      "infoGeneratedByServer": false,
      "pubExpMode": "random",
      "keyFormat": "crt"
      "capabilities": [
      {
        "randPQ": "B.3.2",
        "properties": [
          {
            "modulo": 2048,
            "hashAlg": [
              "SHA2-224"
            ]
          },
          {
            "modulo": 3072,
            "hashAlg": [
              "SHA2-224"
            ]
          }
        ]
      },
      {
        "randPQ": "B.3.4",
        "properties": [
          {
            "modulo": 2048,
            "hashAlg": [
              "SHA2-224"
            ]
          },
          {
            "modulo": 3072,
            "hashAlg": [
              "SHA2-224"
            ]
          }
        ]
      }
    ],
   }

....



[[app-ex-keyGenProbPrime]]
=== Example keyGen with Probable Primes Capabilities JSON Object

The following is an example JSON object advertising support for RSA keyGen with probable primes according to
                        <<FIPS186-4>>, Appendix B.3.3, as well as probably primes with conditions according to
                        <<FIPS186-4>>, Appendix B.3.6. See also
                        <<mode_keyGenFullSet>>.


[align=left,alt=,type=]
....

   {
    "algorithm":"RSA",
    "mode":"keyGen",
    "revision": "1.0",
    "prereqVals":[{"algorithm":"DRBG", "valValue":"123456"}, {"algorithm":"SHA", "valValue":"7890"}],
    "pubExpMode": "fixed",
    "fixedPubExp":"010001",
    "keyFormat": "crt",
    "capabilities":
    [
       {
          "randPQ": "B.3.3",
          "properties": [
            {
              "modulo": 2048,
              "primeTest": [
                "tblC2"
              ]
            },
            {
              "modulo": 3072,
              "primeTest": [
                "tblC2",
                "tblC3"
              ]
            }
          ]
        },
        {
           "randPQ": "B.3.6",
           "properties": [
             {
               "modulo": 2048,
               "primeTest": [
                 "tblC2"
               ]
             },
             {
               "modulo": 3072,
               "primeTest": [
                 "tblC2",
                 "tblC3"
               ]
             }
           ]
         },
     ]
  }

....



[[app-ex-keyGenProvProbPrime]]
=== Example keyGen with Provable Conditional Primes with Probable Factors Capabilities JSON Object

The following is an example JSON object advertising support for RSA keyGen with primes with conditions according to
                        <<FIPS186-4>>, Appendix B.3.5. See also
                        <<mode_keyGenFullSet>>.


[align=left,alt=,type=]
....

   {
      "algorithm": "RSA",
      "mode": "keyGen",
      "revision": "1.0",
      "prereqVals": [{"algorithm": "DRBG", "valValue": "same"}, {"algorithm": "SHA", "valValue": "same"}],
      "pubExpMode" : "fixed",
      "fixedPubExp" : "010001",
      "infoGeneratedByServer": false,
      "keyFormat": "standard",
      "capabilities" :
      [
         {
            "randPQ" : "B.3.5",
            "properties" : [
              {
                "modulo" : 2048,
                "hashAlg" : [
                  "SHA-1",
                  "SHA2-224",
                  "SHA2-256",
                  "SHA2-512"
                ],
                "primeTest": [
                  "tblC2",
                  "tblC3"
                ]
              },
              {
                "modulo" : 3072,
                "hashAlg" : [
                  "SHA-1",
                  "SHA2-224",
                  "SHA2-256",
                  "SHA2-512"
                ],
                "primeTest": [
                  "tblC2"
                ]
              },
              {
                "modulo" : 4096,
                "hashAlg" : [
                  "SHA2-512"
                ],
                "primeTest": [
                  "tblC3"
                ]
              }
            ]
         }
      ]
   }

....



[[app-cap-siggenex]]
=== Example RSA sigGen Capabilities JSON Objects

The following is an example JSON object advertising support for RSA sigGen according to
                    <<FIPS186-4>>.


[align=left,alt=,type=]
....

   {
      "algorithm": "RSA",
      "mode": "sigGen",
      "revision": "1.0",
      "prereqVals": [{"algorithm": "DRBG", "valValue": "same"}, {"algorithm": "SHA", "valValue": "same"}],
      "capabilities" :
      [
         {  "sigType" : "ansx9.31",
            "properties" :
                [
                    { "modulo" : 2048,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                    { "modulo" : 3072,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                    { "modulo" : 4096,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                 ]
         },
         {  "sigType" : "pkcs1v1.5",
            "properties" :
                [
                    { "modulo" : 2048,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                    { "modulo" : 3072,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                    { "modulo" : 4096,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                 ]
         },
         {  "sigType" : "pss",
            "properties" :
                [
                    { "modulo" : 2048,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224",
                          "saltLen" : 28
                        },
                        {
                          "hashAlg" : "SHA2-256",
                          "saltLen" : 32
                        },
                        {
                          "hashAlg" : "SHA2-512",
                          "saltLen" : 64
                        }
                      ]
                    }
                    { "modulo" : 3072,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224",
                          "saltLen" : 28
                        },
                        {
                          "hashAlg" : "SHA2-256",
                          "saltLen" : 32
                        },
                        {
                          "hashAlg" : "SHA2-512",
                          "saltLen" : 64
                        }
                      ]
                    }
                    { "modulo" : 4096,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224",
                          "saltLen" : 28
                        },
                        {
                          "hashAlg" : "SHA2-256",
                          "saltLen" : 32
                        },
                        {
                          "hashAlg" : "SHA2-512",
                          "saltLen" : 64
                        }
                      ]
                    }
                ]
         }
      ],
      "conformances": [
        "SP800-106"
      ]
   }

....



[[app-cap-sigverex]]
=== Example RSA sigVer Capabilities JSON Objects

The following is an example JSON object advertising support for RSA sigVer according to
                    <<FIPS186-4>>.


[align=left,alt=,type=]
....

   {
      "algorithm": "RSA",
      "mode": "sigVer",
      "revision": "1.0",
      "prereqVals": [{"algorithm": "DRBG", "valValue": "123456"}, {"algorithm": "DRBG", "valValue": "654321"}, {"algorithm": "SHA", "valValue": "7890"}],
      "pubExpMode" : "random",
      "capabilities" :
      [
         {  "sigType" : "ansx9.31",
            "properties" :
               [
                    { "modulo" : 2048,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                    { "modulo" : 3072,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                    { "modulo" : 4096,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
               ]
         },
         {  "sigType" : "pkcs1v1.5",
            "properties" :
                [
                    { "modulo" : 2048,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                    { "modulo" : 3072,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                    { "modulo" : 4096,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224"
                        },
                        {
                          "hashAlg" : "SHA2-256"
                        },
                        {
                          "hashAlg" : "SHA2-512"
                        }
                      ]
                    }
                ]
         },
         {  "sigType" : "pss",
            "properties" :
                [
                    { "modulo" : 2048,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224",
                          "saltLen" : 28
                        },
                        {
                          "hashAlg" : "SHA2-256",
                          "saltLen" : 32
                        },
                        {
                          "hashAlg" : "SHA2-512",
                          "saltLen" : 64
                        }
                      ]
                    }
                    { "modulo" : 3072,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224",
                          "saltLen" : 28
                        },
                        {
                          "hashAlg" : "SHA2-256",
                          "saltLen" : 32
                        },
                        {
                          "hashAlg" : "SHA2-512",
                          "saltLen" : 64
                        }
                      ]
                    }
                    { "modulo" : 4096,
                      "hashPair" : [
                        {
                          "hashAlg" : "SHA2-224",
                          "saltLen" : 28
                        },
                        {
                          "hashAlg" : "SHA2-256",
                          "saltLen" : 32
                        },
                        {
                          "hashAlg" : "SHA2-512",
                          "saltLen" : 64
                        }
                      ]
                    }
                 ]
          }
      ],
      "conformances": [
        "SP800-106"
      ]
   }

....



[[app-cap-legacysigverex]]
=== Example RSA legacySigVer Capabilities JSON Objects

The following is an example JSON object advertising support for RSA legacySigVer according to
                    <<FIPS186-4>>.


[align=left,alt=,type=]
....

   {
      "algorithm": "RSA",
      "mode": "legacySigVer",
      "revision": "1.0",
      "prereqVals": [{"algorithm": "DRBG", "valValue": "123456"}, {"algorithm": "DRBG", "valValue": "654321"}, {"algorithm": "SHA", "valValue": "7890"}],
      "capabilities" :
      [
         {  "sigType" : "ansx9.31",
            "properties" :
               [
                  { "modulo" : 2048,
                    "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"]},
                  { "modulo" : 3072,
                    "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"]},
                  { "modulo" : 4096,
                    "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"]}
               ]
         },
         {  "sigType" : "pkcs1v1.5",
            "properties" :
                [
                  { "modulo" : 2048,
                    "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"]},
                  { "modulo" : 3072,
                    "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"]},
                  { "modulo" : 4096,
                    "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"]}
                ]
         },
         {  "sigType" : "pss",
            "properties" :
                [
                  { "modulo" : 2048,
                      "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"],
                      "saltLen" : [28, 32, 64]},
                  { "modulo" : 3072,
                      "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"],
                      "saltLen" : [28, 32, 64]},
                  { "modulo" : 4096,
                      "hashAlg" : ["SHA2-224", "SHA2-256", "SHA2-512"],
                      "saltLen" : [28, 32, 64]}
                 ]
          }
      ]
   }

....



[[app-cap-componentsigverex]]
=== Example signaturePrimitive Capabilities JSON Objects

The following is an example JSON object advertising support for RSA Signature Primitive according to
                    <<FIPS186-4>>.


[align=left,alt=,type=]
....

   {
      "algorithm": "RSA",
      "mode": "signaturePrimitive",
      "revision": "1.0",
      "keyFormat": "crt"
   }

....



[[app-cap-componentdecverex]]
=== Example decryptionPrimitive Capabilities JSON Objects

The following is an example JSON object advertising support for RSA Decryption Primitive according to
                    <<SP800-56B>>.


[align=left,alt=,type=]
....

   {
      "algorithm": "RSA",
      "mode": "decryptionPrimitive",
      "revision": "1.0",
      "prereqVals":
        [
            {"algorithm": "DRBG", "valValue": "123456"},
            {"algorithm": "DRBG", "valValue": "654321"},
            {"algorithm": "SHA", "valValue": "7890"}
        ]
   }

....



[[app-testVect-ex]]
== Example Test Vectors JSON Objects

This appendix contains example JSON objects for test vectors sent from the ACVP server to the crypto module.


[[app-testVectProvProvPrime-ex]]
=== Example Test Vectors for keyGen JSON Objects

[align=left,alt=,type=]
....

 [
   { "acvVersion": <acvp-version> },
   { "vsId": 1133,
     "algorithm": "RSA",
     "mode": "keyGen",
     "revision": "1.0",
     "infoGeneratedByServer": false,
     "pubExpMode" : "random",
     "keyFormat": "standard",
     "testGroups" : [
             {
                 "tgId": 1,
                 "randPQ" : "B.3.2",
                 "modulo" : 2048,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "tests" : [
                    {
                      "tcId" : 1145
                    },
                    {
                      "tcId" : 1146
                    }
                 ]
             },
             {
                "tgId": 2,
                "randPQ": "B.3.2",
                "modulo" : 3072,
                "hashAlg" : "SHA2-256",
                "testType": "AFT",
                "tests": [
                  {
                    "tcId": 1147
                  },
                  {
                    "tcId": 1148
                  }
                ]
              },
             {
                 "tgId": 3,
                 "randPQ" : "B.3.4",
                 "modulo" : 2048,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "tests" : [
                    {
                      "tcId" : 1149
                    },
                    {
                      "tcId" : 1150
                    }
                 ]
             },
             {
                "tgId": 4,
                "randPQ": "B.3.4",
                "modulo" : 3072,
                "hashAlg" : "SHA2-256",
                "testType": "AFT",
                "tests": [
                  {
                    "tcId": 1151
                  },
                  {
                    "tcId": 1152
                  }
                ]
              },
             {
                 "tgId": 5,
                 "randPQ" : "B.3.5",
                 "modulo" : 2048,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "tests" : [
                    {
                      "tcId" : 1153
                    },
                    {
                      "tcId" : 1154
                    }
                 ]
             },
             {
                "tgId": 6,
                "randPQ": "B.3.5",
                "modulo" : 3072,
                "hashAlg" : "SHA2-256",
                "testType": "AFT",
                "tests": [
                  {
                    "tcId": 1155
                  },
                  {
                    "tcId": 1156
                  }
                ]
              },
             {
                 "tgId": 7,
                 "randPQ" : "B.3.6",
                 "modulo" : 2048,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "tests" : [
                    {
                      "tcId" : 1157
                    },
                    {
                      "tcId" : 1158
                    }
                 ]
             },
             {
                "tgId": 8,
                "randPQ": "B.3.6",
                "modulo" : 3072,
                "hashAlg" : "SHA2-256",
                "testType": "AFT",
                "tests": [
                  {
                    "tcId": 1159
                  },
                  {
                    "tcId": 1160
                  }
                ]
              },
             {
                 "tgId": 9,
                 "randPQ" : "B.3.3",
                 "primeTest" : "tblC2",
                 "testType": "KAT",
                 "modulo" : 2048,
                 "tests" : [
                    {
                      "tcId" : 1119,
                      "e" : "df28ab",
                      "pRand" : "",
                      "qRand" : ""
                    },
                    {
                      "tcId" : 1120,
                      "e" : "85a4cf",
                      "pRand" : "",
                      "qRand" : ""
                    }
                 ]
             },
             {
                 "tgId": 10,
                 "randPQ" : "B.3.3",
                 "primeTest" : "tblC2",
                 "testType": "KAT",
                 "modulo" : 3072,
                 "tests" : [
                    {
                      "tcId" : 1121,
                      "e" : "535c97",
                      "pRand" : "",
                      "qRand" : ""
                    }
                 ]
             },
             {
                 "tgId": 11,
                 "randPQ" : "B.3.3",
                 "primeTest" : "tblC3",
                 "testType": "KAT",
                 "modulo" : 2048,
                 "tests" : [
                    {
                      "tcId" : 1122,
                      "e" : "df28ab",
                      "pRand" : "",
                      "qRand" : ""
                    },
                    {
                      "tcId" : 1123,
                      "e" : "85a4cf",
                      "pRand" : "",
                      "qRand" : ""
                    }
                 ]
             },
             {
                 "tgId": 12,
                 "randPQ" : "B.3.3",
                 "primeTest" : "tblC3",
                 "testType": "KAT",
                 "modulo" : 3072,
                 "tests" : [
                    {
                      "tcId" : 1124,
                      "e" : "535c97",
                      "pRand" : "",
                      "qRand" : ""
                    }
                 ]
             }
         ]
      }
   ]

....


Note that this example has "infoGeneratedByServer" set to false. This means the client is responsible for providing the details by running an instance of the appropriate
      Key Generation method for each test. The information returned should match all applicable data in the table in <<vs_tr_keyGen_table>>. For Key Generation method 2 (Appendix B.3.3                     <<FIPS186-4>>) if the public exponent is random, there are two test types. One is a known answer test (KAT) provided by the server resulting in a "pass"/"fail"
    response determining if the input forms a valid key pair. The other, which exists for a fixed public exponent as well, asks the client to generate 10 key pairs ('e', 'p', 'q', 'n', and 'd')
    and the server validates that this pair matches the requirements. Below is an example set of test vectors that would come from the server when the registration includes "infoGeneratedByServer" set to true.



[[app-testVectorsKeygenB34-ex]]
=== Example Test Vectors for keyGen when infoGeneratedByServer is true

[align=left,alt=,type=]
....

[
    { "acvVersion" : <acvp-version> },
    {
        "vsId" : 172,
        "algorithm" : "RSA",
        "mode" : "keyGen",
        "revision": "1.0",
        "testGroups" : [
            {
                "tgId": 1,
                "modulo" : 2048,
                "testType" : "AFT",
                "randPQ" : "B.3.4",
                "pubExpMode" : "fixed",
                "keyFormat" : "standard",
                "fixedPubExp" : "010001",
                "infoGeneratedByServer" : true,
                "hashAlg" : "SHA2-256",
                "tests" : [
                    {
                        "tcId" : 1,
                        "bitlens" : [ 143, 160, 235, 249 ],
                        "seed" : "3CE2DBBFFE28316F8E21BD73201E2D2B060EB14B53C4327627800E07"
                    }, {
                        "tcId" : 2,
                        "bitlens" : [ 169, 316, 277, 168 ],
                        "seed" : "9B6882E5D57519A07042BD7C92192B511E09B07B08941F86C711A1A2"
                    }, {
                        "tcId" : 3,
                        "bitlens" : [ 308, 147, 225, 174 ],
                        "seed" : "03E13C2C88EB9C7789C04FC193AC0A28BE131929539A561C896482A5"
                    }
                ]
            }, {
                "tgId": 2,
                "modulo" : 3072,
                "testType" : "AFT",
                "randPQ" : "B.3.4",
                "pubExpMode" : "fixed",
                "keyFormat" : "standard",
                "fixedPubExp" : "010001",
                "infoGeneratedByServer" : true,
                "hashAlg" : "SHA2-256",
                "tests" : [
                    {
                        "tcId" : 4,
                        "bitlens" : [ 195, 507, 204, 478 ],
                        "seed" : "1C575DDA6CD0B687727C5F1DAC1584C0F7A3869D73AC5FC8D911B6431EC2D9DB"
                    }, {
                        "tcId" : 5,
                        "bitlens" : [ 345, 323, 477, 181 ],
                        "seed" : "802490076AFFA5E58D97505D7C1521A8FFF163296430A1C919B0B8075CE81CAB"
                    }, {
                        "tcId" : 6,
                        "bitlens" : [ 310, 348, 206, 500 ],
                        "seed" : "7017D3909F2B1C01CAF95D2782B66A9D7DA275767E38C125ED32D1EFBCC0E5C0"
                    }
                ]
            }
        ]
    }
]


....



[[app-testVectsigGen-ex]]
=== Example Test Vectors for sigGen JSON Objects

[align=left,alt=,type=]
....

 [
   { "acvVersion": <acvp-version> },
   { "vsId": 1163,
      "algorithm": "RSA",
      "mode": "sigGen",
      "revision": "1.0",
      "testGroups" : [
             {
                 "tgId": 1,
                 "sigType" : "ansx9.31",
                 "hashAlg" : "SHA2-256",
                 "modulo" : 2048,
                 "tests" : [
                    {
                      "tcId" : 1165,
                      "message" : ""
                    }
                 ]
             },
             {
                "tgId": 2,
                "sigType": "ansx9.31",
                "hashAlg" : "SHA2-256",
                "modulo" : 3072,
                "tests": [
                    {
                      "tcId" : 1166,
                      "message" : ""
                    }
                 ]
             },
             {
                "tgId": 3,
                "sigType" : "pkcs1v1.5",
                "hashAlg" : "SHA2-256",
                "modulo" : 2048,
                "tests" : [
                   {
                     "tcId" : 1167,
                     "message" : ""
                   }
                ]
             },
             {
                "tgId": 4,
                "sigType" : "pkcs1v1.5",
                "hashAlg" : "SHA2-256",
                "modulo" : 3072,
                "tests" : [
                    {
                      "tcId" : 1168,
                      "message" : ""
                    }
                 ]
             },
             {
                "tgId": 5,
                "sigType" : "pss",
                "hashAlg" : "SHA2-256",
                "modulo" : 2048,
                "tests" : [
                    {
                      "tcId" : 1169,
                      "saltLen" : 20,
                      "message" : ""
                    }
                 ]
             },
             {
                "tgId": 6,
                "sigType" : "pss",
                "hashAlg" : "SHA2-256",
                "modulo" : 3072,
                "tests" : [
                    {
                      "tcId" : 1170,
                      "saltLen" : 20,
                      "message" : ""
                    }
                 ]
             },
             {
                 "tgId": 7,
                 "sigType" : "ansx9.31",
                 "hashAlg" : "SHA2-256",
                 "modulo" : 2048,
                 "conformance": "SP800-106",
                 "tests" : [
                    {
                      "tcId" : 11165,
                      "message" : ""
                    }
                 ]
             },
             {
                "tgId": 8,
                "sigType": "ansx9.31",
                "hashAlg" : "SHA2-256",
                "modulo" : 3072,
                "conformance": "SP800-106",
                "tests": [
                    {
                      "tcId" : 11166,
                      "message" : ""
                    }
                 ]
             },
             {
                "tgId": 9,
                "sigType" : "pkcs1v1.5",
                "hashAlg" : "SHA2-256",
                "modulo" : 2048,
                "conformance": "SP800-106",
                "tests" : [
                   {
                     "tcId" : 11167,
                     "message" : ""
                   }
                ]
             },
             {
                "tgId": 10,
                "sigType" : "pkcs1v1.5",
                "hashAlg" : "SHA2-256",
                "modulo" : 3072,
                "conformance": "SP800-106",
                "tests" : [
                    {
                      "tcId" : 11168,
                      "message" : ""
                    }
                 ]
             },
             {
                "tgId": 11,
                "sigType" : "pss",
                "hashAlg" : "SHA2-256",
                "modulo" : 2048,
                "conformance": "SP800-106",
                "tests" : [
                    {
                      "tcId" : 11169,
                      "saltLen" : 20,
                      "message" : ""
                    }
                 ]
             },
             {
                "tgId": 12,
                "sigType" : "pss",
                "hashAlg" : "SHA2-256",
                "modulo" : 3072,
                "conformance": "SP800-106",
                "tests" : [
                    {
                      "tcId" : 11170,
                      "saltLen" : 20,
                      "message" : ""
                    }
                 ]
             }
       ]
    }
  ]

....



[[app-testVectsigVer-ex]]
=== Example Test Vectors for sigVer JSON Objects

[align=left,alt=,type=]
....

 [
   { "acvVersion": <acvp-version> },
   { "vsId": 1173,
      "algorithm": "RSA",
      "mode": "sigVer",
      "revision": "1.0",
      "testGroups" : [
             {
                 "tgId": 1,
                 "sigType" : "ansx9.31",
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "modulo" : 2048,
                 "e" : "166f67",
                 "n" : "",
                 "tests" : [
                    {
                      "tcId" : 1174,
                      "message" : "",
                      "signature" : ""
                    }
                 ]
             },
             {
                 "tgId": 2,
                 "sigType" : "ansx9.31",
                 "modulo" : 3072,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "e" : "89ca09",
                 "n" : "",
                 "tests" : [
                    {
                      "tcId" : 1175,
                      "message" : "",
                      "signature" : ""
                    }
                 ]
             },
             {
                 "tgId": 3,
                 "sigType" : "pkcs1v1.5",
                 "modulo" : 2048,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "e" : "49d2a1",
                 "n" : "",
                 "tests" : [
                    {
                      "tcId" : 1176,
                      "message" : "",
                      "signature" : ""
                    }
                 ]
             },
             {
                "tgId": 4,
                "sigType" : "pkcs1v1.5",
                "modulo" : 3072,
                "hashAlg" : "SHA2-256",
                "testType": "AFT",
                "e" : "ac6db1",
                "n" : "",
                "tests" : [
                    {
                      "tcId" : 1177,
                      "message" : "",
                      "signature" : ""
                    }
                 ]
             },
             {
                 "tgId": 5,
                 "sigType" : "pss",
                 "modulo" : 2048,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "e" : "10e43f",
                 "n" : "",
                 "tests" : [
                    {
                      "tcId" : 1178,
                      "message" : "",
                      "signature" : ""
                    }
                 ]
             },
             {
                "tgId": 6,
                "sigType" : "pss",
                "modulo" : 3072,
                "hashAlg" : "SHA2-512",
                "testType": "AFT",
                "e" : "fe3079",
                "n" : "",
                "tests" : [
                    {
                      "tcId" : 1179,
                      "message" : "",
                      "signature" : ""
                    }
                 ]
             },
             {
                 "tgId": 7,
                 "sigType" : "ansx9.31",
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "modulo" : 2048,
                 "e" : "166f67",
                 "n" : "",
                 "conformance": "SP800-106",
                 "tests" : [
                    {
                      "tcId" : 11174,
                      "message" : "",
                      "randomValue": "ab676ab76a76b...",
                      "randomValueLen": 1024,
                      "signature" : ""
                    }
                 ]
             },
             {
                 "tgId": 8,
                 "sigType" : "ansx9.31",
                 "modulo" : 3072,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "e" : "89ca09",
                 "n" : "",
                 "conformance": "SP800-106",
                 "tests" : [
                    {
                      "tcId" : 11175,
                      "message" : "",
                      "randomValue": "ab61bc76ab76a76b...",
                      "randomValueLen": 1024,
                      "signature" : ""
                    }
                 ]
             },
             {
                 "tgId": 9,
                 "sigType" : "pkcs1v1.5",
                 "modulo" : 2048,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "e" : "49d2a1",
                 "n" : "",
                 "conformance": "SP800-106",
                 "tests" : [
                    {
                      "tcId" : 11176,
                      "message" : "",
                      "randomValue": "ab676ab761abca76b...",
                      "randomValueLen": 1024,
                      "signature" : ""
                    }
                 ]
             },
             {
                "tgId": 10,
                "sigType" : "pkcs1v1.5",
                "modulo" : 3072,
                "hashAlg" : "SHA2-256",
                "testType": "AFT",
                "e" : "ac6db1",
                "n" : "",
                "conformance": "SP800-106",
                "tests" : [
                    {
                      "tcId" : 11177,
                      "message" : "",
                      "randomValue": "ab676abab6283676a76b...",
                      "randomValueLen": 1024,
                      "signature" : ""
                    }
                 ]
             },
             {
                 "tgId": 11,
                 "sigType" : "pss",
                 "modulo" : 2048,
                 "hashAlg" : "SHA2-256",
                 "testType": "AFT",
                 "e" : "10e43f",
                 "n" : "",
                 "conformance": "SP800-106",
                 "tests" : [
                    {
                      "tcId" : 11178,
                      "message" : "",
                      "randomValue": "ab67a6786b876c6ab76a76b...",
                      "randomValueLen": 1024,
                      "signature" : ""
                    }
                 ]
             },
             {
                "tgId": 12,
                "sigType" : "pss",
                "modulo" : 3072,
                "hashAlg" : "SHA2-512",
                "testType": "AFT",
                "e" : "fe3079",
                "n" : "",
                "conformance": "SP800-106",
                "tests" : [
                    {
                      "tcId" : 11179,
                      "message" : "",
                      "randomValue": "ab6a9b8b6a75ba76ab76a76b...",
                      "randomValueLen": 1024,
                      "signature" : ""
                    }
                 ]
             }
          ]
      }
  ]

....


Note: The ACVP server does retain additional context for each test vector sent to the client in order to verify the results when submitted for validation.
    These may include the public key n, the public exponent e, the private key d for each test. Note also that each test for which msg is not between 0 and n - 1 should fail.


[[app-testVectlegacySigVer-ex]]
=== Example Test Vectors for legacySigVer JSON Objects

The format and structure of legacySigVer Test Vector JSON Objects is identical to those for the Test Vectors for sigVer JSON Objects.


[[app-testsignaturePrimitive-ex]]
=== Example Test Vectors for signaturePrimitive JSON Objects

[align=left,alt=,type=]
....

 [
   { "acvVersion": <acvp-version> },
   { "vsId": 1193,
      "algorithm": "RSA",
      "mode": "signaturePrimitive",
      "revision": "1.0",
      "keyFormat": "standard",
      "testGroups" : [
             {
                 "tgId": 1,
                 "tests" : [
                    {
                      "tcId" : 1194,
                      "n" : "",
                      "d" : "",
                      "message" : ""
                    },
                    {
                      "tcId" : 1195,
                      "n" : "",
                      "d" : "",
                      "message" : ""
                    }
                 ]
             }
        ]
    }
  ]

....


Note: The ACVP server does retain additional context for each test vector sent to the client in order to verify the results when submitted for validation. These may include the public key 'n', the public exponent 'e', the private key 'd' for each test.
    Note also that each test for which 'msg' is not between '0' and 'n - 1' should fail.


[[app-testdecryptionPrimitive-ex]]
=== Example Test Vectors for decryptionPrimitive JSON Objects

[align=left,alt=,type=]
....

 [
   { "acvVersion": <acvp-version> },
   { "vsId": 1194,
      "algorithm": "RSA",
      "mode": "decryptionPrimitive",
      "revision": "1.0",
      "testGroups" : [
             {
                 "tgId": 1,
                 "totalTests": 2,
                 "totalFailingTests": 1,
                 "tests" : [
                    {
                      "tcId" : 1,
                      "resultsArray": [
                      {
                        "cipherText" : ""
                      },
                      {
                        "cipherText" : ""
                      }
                    }
                 ]
             }
       ]
    }
  ]

....


Note: The ACVP server does retain additional context for each test vector sent to the client in order to verify the results when submitted for validation. These may include the public key 'n', the public exponent 'e', the private key 'd' for each test.
    Note also that each test for which 'cipherText' is not between '0' and 'n - 1' should fail.


[[app-results-ex]]
== Example Test Results JSON Objects


[[app-keyGen-results-ex]]
=== Example keyGen Test Results JSON Objects

The following is a example JSON object for RSA keyGen test results sent from the crypto module to the ACVP server.

[align=left,alt=,type=]
....

             [
                { "acvVersion": <acvp-version> },
                { "vsId": 1133,
                    "algorithm": "RSA",
                    "mode": keyGen,
                    "revision": "1.0",
                    "testGroups": [
                    {
                      "tgId": 1,
                      "tests": [
                        {
                          "tcId" : 1111,
                          "e" : "10000021",
                          "seed" : "af152e46b479af86d2eecb2c8e503dc90954866403e4be1d2d716b2a",
                          "bitlens" : [312, 145, 144, 338],
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                        {
                          "tcId" : 1112,
                          "e" : "10000021",
                          "seed" : "4f317e61d35e6f7dc19d038b9da897e64c3e7706b49d30ab2dda32ef",
                          "bitlens" : [320, 159, 192, 222],
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        }
                      ]
                    },
                    {
                      "tgId": 2,
                      "tests": [
                        {
                          "tcId" : 1113,
                          "e" : "10000021",
                          "seed" : "3bbc67c71012c63e563580555acb96023106a8d5247d6b27d2b20475681bdcb",
                          "bitlens" : [448, 281, 240, 469],
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                        {
                          "tcId" : 1114,
                          "e" : "10000021",
                          "seed" : "6b6b99c71902a6e23cd941494876958fe816e8e2f587b4f05f24e8f674b9b10a",
                          "bitlens" : [504, 238, 440, 185],
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                      ]
                    },
                    {
                      "tgId": 3,
                      "tests": [
                        {
                          "tcId" : 1115,
                          "e" : "10000021",
                          "seed" : "e664bc8c8e09ca232ae38171efc8891085da42fda83463febdc37241",
                          "bitlens" : [232, 220, 336, 141],
                          "xP" : "",
                          "p" : "",
                          "xQ" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                        {
                          "tcId" : 1116,
                          "e" : "10000021",
                          "seed" : "242d14906b68f64a8836c27ce91913b042098806d2a8566035805c48",
                          "bitlens" : [272, 205, 296, 157],
                          "xP" : "",
                          "p" : "",
                          "xQ" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                      ]
                    },
                    {
                      "tgId": 4,
                      "tests": [
                        {
                          "tcId" : 1117,
                          "e" : "10000021",
                          "seed" : "6710f90af3479a276ee2f1500d444c944e48da1d5425b4afc64b058e509ddbbc",
                          "bitlens" : [512, 196, 560, 180],
                          "xP" : "",
                          "p" : "",
                          "xQ" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                        {
                          "tcId" : 1118,
                          "e" : "10000021",
                          "seed" : "7da214c48281d4647edc13973594780a0e6ddfa089d730e47e9e4dd53a85b6e7",
                          "bitlens" : [512, 182, 504, 222],
                          "xP" : "",
                          "p" : "",
                          "xQ" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                      ]
                    },
                    {
                      "tgId": 5,
                      "tests": [
                        {
                          "tcId" : 1135,
                          "e" : "10000021",
                          "bitlens" : [224, 195, 352, 142],
                          "xP1" : "57c9a2986fc7e69e835fd2847ed0a0d6983d8921130628cf86c8d811",
                          "xP2" : "7254d6c998a84230ff25531243b8a6d0e05141ca56fba6f0f",
                          "xP" : "",
                          "p" : "",
                          "xQ1" : "7468d10e69a14b00ec128f7dd19f6f7317bcf96a97989cd5a9051b3ca75268362acf3918cd810093026606fd",
                          "xQ2" : "20b8c2bae262b13e912094c937c4e8cfa573",
                          "xQ" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                        {
                          "tcId" : 1137,
                          "e" : "10000021",
                          "bitlens" : [320, 315, 320, 277],
                          "xP1" : "b95de5dee2eb96d9e4a5e6a87d0630c0439febc21cfdd9c6f973b8f7501a1919211d120fea077677",
                          "xP2" : "658ab200f20d118be4d915d5791f6397ee0ee2134b2460b1e10dce4911756eebd8cba8fc9763963",
                          "xP" : "",
                          "p" : "",
                          "xQ1" : "f21ebb87473fdcff5f7a4fe439027a6c1853cd94a3b7209d1fdb3208c20f2625f9ae12e13ae9ee45",
                          "xQ2" : "19be84f228b1576165c13970c88939e7a027f29c3fc33542006dbead17a7e1e72d8bf9",
                          "xQ" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                      ]
                    },
                    {
                      "tgId": 6,
                      "tests": [
                        {
                          "tcId" : 1145,
                          "seed" : "45406f8f889763b751c841880a5fdeec82446c08c896b5f5d70edb8d",
                          "e" : "10000021",
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                        {
                          "tcId" : 1147,
                          "seed" : "6ba71c221702a1a6805c3a421e2af234129b429002f640b9834c41e9479a7f91",
                          "e" : "10000021",
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                      ]
                    },
                    {
                      "tgId": 7,
                      "tests": [
                        {
                          "tcId" : 1119,
                          "testPassed": true
                        },
                        {
                          "tcId" : 1120,
                          "testPassed": false
                        },
                      ]
                    },
                    {
                      "tgId": 8,
                      "tests": [
                        {
                          "tcId" : 1121,
                          "testPassed": true
                        },
                        {
                          "tcId" : 1122,
                          "testPassed": false
                        },
                      ]
                    },
                    {
                      "tgId": 9,
                      "tests": [
                        {
                          "tcId" : 1123,
                          "testPassed": false
                        },
                        {
                          "tcId" : 1124,
                          "testPassed": false
                        },
                      ]
                    },
                    {
                      "tgId": 10,
                      "tests": [
                        {
                          "tcId" : 1125,
                          "testPassed": false
                        },
                        {
                          "tcId" : 1126,
                          "testPassed": false
                        },
                      ]
                    },
                    {
                      "tgId": 11,
                      "tests": [
                        {
                          "tcId" : 1129,
                          "e" : "df28ab",
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                        {
                          "tcId" : 1130,
                          "e" : "e66d81",
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                      ]
                    },
                    {
                      "tgId": 12,
                      "tests": [
                        {
                          "tcId" : 1131,
                          "e" : "535c97",
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        },
                        {
                          "tcId" : 1132,
                          "e" : "535c97",
                          "p" : "",
                          "q" : "",
                          "n" : "",
                          "d" : ""
                        }
                      ]
                    }
                 ]
                }
             ]


....



[[app-sigGen-results-ex]]
=== Example sigGen Test Results JSON Objects

The following is a example JSON object for RSA sigGen test results sent from the crypto module to the ACVP server.

[align=left,alt=,type=]
....

             [
                { "acvVersion": <acvp-version> },
                { "vsId": 1163,
                  "algorithm": "RSA",
                  "mode": "sigGen",
                  "revision": "1.0",
                  "testGroups": [
                    {
                      "tgId": 1,
                      "tests": [
                        {
                          "tcId" : 1165,
                          "e" : "f3e7af",
                          "n" :"",
                          "signature" : ""
                        },
                        {
                          "tcId" : 1166,
                          "e" : "ebda09",
                          "n" : "",
                          "signature" : ""
                        },
                      ]
                    },
                    {
                      "tgId": 2,
                      "tests": [
                        {
                          "tcId" : 1167,
                          "e" : "260445",
                          "n" : "",
                          "signature" : ""
                        },
                        {
                          "tcId" : 1168,
                          "e" : "eaf05d",
                          "n" : "",
                          "signature" : ""
                        },
                      ]
                    },
                    {
                      "tgId": 3,
                      "tests": [
                        {
                          "tcId" : 1169,
                          "e" : "86c94f",
                          "n" : "",
                          "signature" : ""
                        },
                        {
                          "tcId" : 1170,
                          "e" : "1415a7",
                          "n" : "",
                          "signature" : ""
                        }
                      ]
                    },
                    {
                      "tgId": 7,
                      "tests": [
                        {
                          "tcId" : 11165,
                          "e" : "f3e7af",
                          "n" :"",
                          "signature" : "",
                          "randomValue": "ab67a6b7a6b5ab67a5...",
                          "randomValueLen": 1024
                        },
                        {
                          "tcId" : 11166,
                          "e" : "ebda09",
                          "n" : "",
                          "signature" : "",
                          "randomValue": "ab67a6b67a86ab87687667b7a6b5ab67a5...",
                          "randomValueLen": 1024
                        },
                      ]
                    },
                    {
                      "tgId": 8,
                      "tests": [
                        {
                          "tcId" : 11167,
                          "e" : "260445",
                          "n" : "",
                          "signature" : ""
                          "randomValue": "ab67ab7a897ba897a6b7a6b5ab67a5...",
                          "randomValueLen": 1024
                        },
                        {
                          "tcId" : 11168,
                          "e" : "eaf05d",
                          "n" : "",
                          "signature" : "",
                          "randomValue": "ab67a6b7a6b5ab67aa768b6a87b65...",
                          "randomValueLen": 1024
                        },
                      ]
                    },
                    {
                      "tgId": 9,
                      "tests": [
                        {
                          "tcId" : 11169,
                          "e" : "86c94f",
                          "n" : "",
                          "signature" : "",
                          "randomValue": "ab67a6b7a6b5ab67aa768b6a87ba67ba6b6a754a3265...",
                          "randomValueLen": 1024
                        },
                        {
                          "tcId" : 11170,
                          "e" : "1415a7",
                          "n" : "",
                          "signature" : "",
                          "randomValue": "ab67a6ab67ab4ab23b7a6b5ab67aa768b6a87b65...",
                          "randomValueLen": 1024
                        }
                      ]
                    }
                 ]
             }
           ]


....



[[app-sigVer-results-ex]]
=== Example sigVer Test Results JSON Objects

The following is a example JSON object for RSA sigVer test results sent from the crypto module to the ACVP server.

[align=left,alt=,type=]
....

 [
    { "acvVersion": <acvp-version> },
    { "vsId": 1173,
      "algorithm": "RSA",
      "mode": "sigVer",
      "revision": "1.0",
      "testGroups" : [
          {
            "tgId": 1,
            "tests": [
                {
                  "tcId" : 1174,
                  "testPassed": false
                },
                {
                  "tcId" : 1175,
                  "testPassed": false
                }
              ]
          },
          {
            "tgId": 2,
            "tests": [
                {
                  "tcId" : 1176,
                  "testPassed": true
                },
                {
                  "tcId" : 1177,
                  "testPassed": false
                }
              ]
          },
          {
            "tgId": 3,
            "tests": [
                {
                  "tcId" : 1178,
                  "testPassed": false
                },
                {
                  "tcId" : 1179,
                  "testPassed": false
                }
              ]
          }
          {
            "tgId": 7,
            "tests": [
                {
                  "tcId" : 11174,
                  "testPassed": false
                },
                {
                  "tcId" : 11175,
                  "testPassed": false
                }
              ]
          },
          {
            "tgId": 8,
            "tests": [
                {
                  "tcId" : 11176,
                  "testPassed": true
                },
                {
                  "tcId" : 11177,
                  "testPassed": false
                }
              ]
          },
          {
            "tgId": 9,
            "tests": [
                {
                  "tcId" : 11178,
                  "testPassed": true
                },
                {
                  "tcId" : 11179,
                  "testPassed": false
                }
              ]
          }
        ]
    }
]


....



[[app-legacySigVer-results-ex]]
=== Example legacySigVer Test Results JSON Objects

The format and structure of the RSA legacySigVer Test Results JSON Objects are identical to those of the RSA sigVer Test Results JSON Objects.


[[app-componentsig-results-ex]]
=== Example signaturePrimitive Test Results JSON Objects

The following is a example JSON object for RSA signaturePrimitive test results sent from the crypto module to the ACVP server.

[align=left,alt=,type=]
....

[
    { "acvVersion": <acvp-version> },
    { "vsId": 47,
      "algorithm": "RSA",
      "mode": "signaturePrimitive",
      "revision": "1.0",
      "testGroups" : [
        {
          "tgId": 1,
          "tests": [
            {
              "tcId" : 1194,
              "signature" : ""
            },
            {
              "tcId" : 1195,
              "testPassed": false
            }
          ]
        }
      ]
    }
]


....



[[app-componentdec-results-ex]]
=== Example decryptionPrimitive Test Results JSON Objects

The following is a example JSON object for RSA decryptionPrimitive test results sent from the crypto module to the ACVP server.

[align=left,alt=,type=]
....

[
    { "acvVersion": <acvp-version> },
    { "vsId": 48,
      "algorithm": "RSA",
      "mode": "decryptionPrimitive",
      "revision": "1.0",
      "testGroups" : [
        {
          "tgId": 1,
          "tests": [
            {
              "tcId" : 1196,
              "e" : "010001",
              "n" : "",
              "plainText" : ""
            },
            {
              "tcId" : 1197,
              "e" : "010001",
              "n" : "",
              "testPassed": false
            }
          ]
        }
      ]
    }
]


....
